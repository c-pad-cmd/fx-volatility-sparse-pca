...
<img src="docs/figures/ic_vs_k.png" width="560" alt="AIC/CAIC/SBC/ICOMP vs. number of components k">

All four criteria track each other almost exactly across k (they share the
same lack-of-fit term and differ only by a small penalty), including the
secondary dip at k=7 called out below, which is why they always agree on a
minimum even though the curve itself isn't smooth.

Plotting the SPC1 and SPC2 loadings against each other instead of separately
shows more than the developed/emerging split visible in the stem plots above:

<img src="docs/figures/spc1_spc2_biplot.png" width="560" alt="Currency loadings: SPC1 vs SPC2, showing a haven-vs-high-beta split on SPC2">

SPC2 cuts the panel a different way. GBP, ZAR, KRW, MXN, TRY, and IDR (one
developed currency plus five emerging ones) all load strongly positive on
SPC2, while JPY, ILS, and SGD load negative; the EUR-block currencies, CAD,
NOK, PLN, CZK, and BRL sit near zero. That reads as a haven/low-yielder axis
(JPY, ILS, SGD) versus a higher-beta bloc, cutting across the
developed/emerging line that SPC1 draws. This is fully reproducible (verified
across three separate runs, byte-identical results; see the note on
`random_state` below), not an artifact of a particular fit.
...
exponential specification improves on the plain model here.

<img src="docs/figures/garch_family_comparison.png" width="700" alt="GARCH-family model comparison, delta AIC/BIC/ICOMP vs. the best model, per SPC">

(Plotted as each criterion's gap from its own best value, not the raw
AIC/BIC/ICOMP magnitude: the three model families differ by only a few
points out of a shared ~11,000 baseline, so raw values look identical at
that scale.)

The GARCH(p,q) grid search (p,q = 1..4) on SPC1 confirms (1,1) as the
AIC-best order too; every larger model overfits.

<img src="docs/figures/spc1_garch_grid_heatmap.png" width="480" alt="GARCH(p,q) grid search AIC heatmap for SPC1">

The fitted SPC1 GARCH(1,1) lands almost exactly
...
Full CSV tables (`results/*.csv`) and all 12 figures are regenerated by
rerunning the pipeline (deterministically, on a fixed environment; see the
...
│   └── plotting.py            # stem/biplots, ACF/PACF, IC/GARCH comparison plots
...
`results/` (git-ignored; regenerate anytime by rerunning). `fit_sparse_pca`
and `select_number_of_components` both pin `random_state=0` by default, so
reruns on the same machine with the same library versions are fully
reproducible byte-for-byte (verified: three consecutive runs produced
identical CSVs and PNGs). Numbers can still differ across *environments*,
since `sklearn.SparsePCA`'s coordinate descent is a non-convex optimization
and different sklearn/numpy/BLAS versions can take slightly different
floating-point paths through it even with the same seed; that's the likely
reason the committed `docs/figures/` snapshot and CSVs won't necessarily
match a fresh run on a different setup. Runtime is well under a minute (~10s
on a laptop), mostly spent fitting the GARCH(p, q) grid. `docs/figures/`
holds a fixed, committed snapshot of the figures referenced in
...
recover erratically across k: in the committed run (see the IC-vs-k plot
above), there's a clean global minimum at k=2 but also a secondary, smaller
dip at k=7 before the curve rises again through k=8-10. Treat the "best k"
output as indicative, not as precise as the original inverse-power-method's
deflation-based sweep, which updates one component at a time from a shared
basis.
